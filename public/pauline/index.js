...
window.onload = async function () {
  ...
  d3.json("../data/songWithGenre.json").then((data) => {
    ...
    var dateScale = d3.scaleTime().domain(domain).range([1, width]);
    ...
    // Add Y axis
    var y_length = d3.scaleLinear().domain([90, 300]).range([height, 0]);

    svg.append("g").call(d3.axisLeft(y_length));
    ...
    const elseSongs = dataFilter.filter(
      (d) =>
        !(
          d.genre.includes("Pop") ||
          d.genre.includes("Rock") ||
          d.genre.includes("Metal")
        ),
    );
    ...
    svg
      ...
      .append("circle")
      .attr("cx", function (d) {
        return dateScale(parseTime(d.publicationDate));
      })
      .attr("cy", function (d) {
        return y_length(d.length);
      })
      .attr("r", function (d) {
    ...
    svg
      ...
      .attr("transform", function (d) {
        return (
          "translate(" +
          dateScale(parseTime(d.publicationDate)) +
          "," +
          y_length(d.length) +
          ")"
        );
