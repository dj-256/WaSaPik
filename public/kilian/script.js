...
function getAvgBpm(countryStats, countryId) {
  return countryStats.countries[countryId].average_bpm;
}

function getAvgLyrics(countryStats, countryId) {
  return countryStats.countries[countryId].average_lyrics;
}

function setColorScale(countryStats, filter) {
  if (filter === "bpm")
    return d3
  ...
  return d3
    .scaleSequential(d3.interpolateViridis)
    .domain([countryStats.min_lyrics, countryStats.max_lyrics]);
}

async function loadMap(filter) {
  ...
  let colorScale = setColorScale(countryStats, filter);

  d3.json(
    "https://raw.githubusercontent.com/epistler999/GeoLocation/master/world.json",
  ).then((data) => {
    svg
      .append("g")
      .selectAll("path")
      ...
      .attr("fill", function (d) {
        ...
        if (!countryStats.countries[countryId]) return;

        return colorScale(
          filter === "bpm"
            ? getAvgBpm(countryStats, countryId)
            : getAvgLyrics(countryStats, countryId),
        );
      })
      ...
      .on("click", function (d) {
        ...
        document.getElementById("lyrics").innerText =
          Math.round(averageLyrics) + " words";

        console.log(countryStat.genres);
        document.getElementById("genre-data").innerText =
          countryStat.genre_data;
        document.getElementById("genres-count").innerText =
          countryStat.genres_count;

        // Graph
        var chartSvg = d3.select("#country-char");
        chartSvg.selectAll("*").remove();

        if (countryStat.genres_count == 0) return;
